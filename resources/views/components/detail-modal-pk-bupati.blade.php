...
<script>
    // Fungsi untuk membuka modal detail dengan data
    function openDetailModalWithData(data) {
        // Isi informasi dasar
        document.getElementById('detailNo').textContent = data.no || '-';
        document.getElementById('detailTahun').textContent = '2025';
        document.getElementById('detailTahunHeader').textContent = '2025';
        ...
        // Isi data triwulan
        for (let i = 1; i <= 4; i++) {
            document.getElementById('detailTargetTW' + i).textContent = data['targetTW' + i] || '-';
            document.getElementById('detailRealisasiTW' + i).textContent = data['realisasiTW' + i] || '-';
            document.getElementById('detailPaguTW' + i).textContent = data['paguAnggaranTW' + i] || '-';
            document.getElementById('detailRealisasiAnggaranTW' + i).textContent = data['realisasiAnggaranTW' + i] || '-';
        }

        // Reset tabs ke triwulan 1
        document.querySelectorAll('.tabcontent').forEach(tab => {
            tab.classList.add('hidden');
        });
        document.getElementById('detailTriwulan1').classList.remove('hidden');

        document.querySelectorAll('.tablinks').forEach(btn => {
            btn.classList.remove('bg-gray-200');
            btn.classList.add('bg-gray-100');
        });
        document.querySelector('.tablinks').classList.remove('bg-gray-100');
        document.querySelector('.tablinks').classList.add('bg-gray-200');

        // Buka modal
        openModal('detailModal');
    }

    // Fungsi untuk beralih tab detail
