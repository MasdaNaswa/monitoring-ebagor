    ...
    <main class="flex-1 px-4 md:px-8 py-6 bg-[#F8FAFC]">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6 px-4 md:px-6 py-4">
            <div class="flex flex-col md:flex-row items-center gap-4">
                ...
                @php
                    $startYear = 2024; // tahun awal sistem
                    $currentYear = now()->year;
                    $selectedYear = request()->get('year', $currentYear); // ambil dari query string atau default sekarang
                ...
                <form method="GET" class="flex items-center gap-3">
                    ...
                    <select name="semester" id="semesterFilter"
                        class="py-2 px-3 rounded border border-gray-300 hover:border-primary transition focus:outline-none focus:ring-2 focus:ring-blue-200 text-sm md:text-base"
                        onchange="this.form.submit()">
                        <option value="1" {{ (request()->get('semester', '1') == '1') ? 'selected' : '' }}>I</option>
                        <option value="2" {{ (request()->get('semester', '1') == '2') ? 'selected' : '' }}>II</option>
                    </select>
                </form>
...
<script>
    ...
    let currentYear = '{{ request()->get("year", now()->year) }}';
    let currentSemester = '{{ request()->get("semester", "1") }}';
    ...
    function populateEditForm(index) { 
        ...
        set('editId', index); 
        set('editNo', data.no); 
        set('editTarget2025', data.target2025); 
        set('editSatuan', data.satuan); 
        set('editProgram', data.program); 
        set('editAnalisisEvaluasi', data.analisisEvaluasi); 
        set('editPenanggungJawab', data.penanggungJawab); 
        for (let i = 1; i <= 4; i++) { 
            set('editTargetTW' + i, data['targetTW' + i]); 
            set('editRealisasiTW' + i, data['realisasiTW' + i]); 
            set('editPaguAnggaranTW' + i, data['paguAnggaranTW' + i]); 
            set('editRealisasiAnggaranTW' + i, data['realisasiAnggaranTW' + i]); 
        } 
    }
    ...
    function showDetail(index) { 
        ...
        set('detailNo', data.no); 
        set('detailTahun', data.tahun ?? currentYear); 
        set('detailSasaranStrategis', data.sasaranStrategis); 
        set('detailIndikatorKinerja', data.indikatorKinerja); 
        set('detailTarget2025', data.target2025); 
        set('detailSatuan', data.satuan); 
        set('detailProgram', data.program); 
        set('detailAnalisisEvaluasi', data.analisisEvaluasi); 
        set('detailPenanggungJawab', data.penanggungJawab); 
        for (let i = 1; i <= 4; i++) { 
            set('detailTargetTW' + i, data['targetTW' + i]); 
            set('detailRealisasiTW' + i, data['realisasiTW' + i]); 
            set('detailPaguTW' + i, data['paguAnggaranTW' + i]); 
            set('detailRealisasiAnggaranTW' + i, data['realisasiAnggaranTW' + i]); 
        } 
    ...
    function deleteData(index) { 
        ...
        const e = document.getElementById('hapusItem'); 
        if (e) e.textContent = `No ${data.no} - ${data.sasaranStrategis}`; 
    }
</script>
@endsection
